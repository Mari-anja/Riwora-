...
import { fetchPendingCustomers } from './api';  // ✅ Create this API function
import { ChevronsRight, ChevronLeft } from "lucide-react-native";

type PendingCustomersProps = NativeStackScreenProps<RootStackParamList, 'PendingCustomers'>;

const PendingCustomers: React.FC<PendingCustomersProps> = ({ navigation }) => {
    ...
    useEffect(() => {
        const getCustomers = async () => {
            ...
            const data = await fetchPendingCustomers(storedUserId);  // ✅ Fetch pending customers
            setCustomers(data);
            setLoading(false);
        };
    ...
    return (
        ...
        <View style={{ width: 32 }} /> 
            ...
            {loading ? (
                ...
                <Text style={styles.noCustomers}>No pending customers found.</Text>
            )}

            <BottomNavigation navigation={navigation} activeTab="PendingCustomers" />
        </View>
        </SafeAreaView>
    );
};
